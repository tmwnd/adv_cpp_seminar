...
\begin{frame}[fragile]{Range Concepts}
    \begin{itemize}
        \item<1-> \mintinline{c++}{std::ranges::input_range}
            \onslide<2->{\begin{itemize} \item \mintinline{c++}{std::cin} \end{itemize}}
            % \item<2-> std::ranges::forward\_range
            % \only<2->{\begin{itemize} \item std::forward\_list \end{itemize}}
        \item<3-> \mintinline{c++}{std::ranges::bidirectional_range}
            \onslide<4->{\begin{itemize} \item \mintinline{c++}{std::list} \end{itemize}}
        \item<5-> \mintinline{c++}{std::random_access_range}
            \onslide<6->{\begin{itemize} \item \mintinline{c++}{std::unordered_map} \end{itemize}}
            % \item<5-> std::ranges::contiguous\_range
            % \only<5->{\begin{itemize} \item std::vector \end{itemize}}
        \item<7-> \mintinline{c++}{std::ranges::sized_range}
            \onslide<8->{\begin{itemize} \item \mintinline{c++}{std::vector} \end{itemize}}
    \end{itemize}
\end{frame}
