...
class view_entwickle : public std::ranges::view_base {
   ...
   public:
    ...
    ~view_entwickle() = default;
    view_entwickle(R &&r)
        : data_members{new data_members_t{std::forward<R>(r)}} {}

    iterator_type begin() const { return std::begin(data_members->r); }

    iterator_type end() const { return std::end(data_members->r); }
};
...
int main() {
    ...
    auto view = pokemon | view::entwickle |
                std::views::filter([](const auto &p) { return p.id != 0; });
